...
app.post('/chat', async (req, res) => {
    if (!process.env.OPENAI_API_KEY)
        return res.status(500).end('Please add your OPENAI_API_KEY') // CLI command: coho set-env OPENAI_API_KEY 'XXX'
    const { ask } = req.body
    ...
    if (cachedAnswer) {
        ...
        } = await callOpenAiApi(ask)
        console.log('ask222', 'text222')
